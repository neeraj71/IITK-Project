...
for(k in c(1:length(sig))){
  ...
  for(n in c(1:length(X))){
    ...
    T[,1]=exp(-rowSums((sweep(G,2,G[n,],FUN="-"))^2)/(sig[k])^2)
    D[,1]=sqrt(1+1-2*T)
    Z=1/D
    Z[n]=0
  ...
  for(j in c(1:length(H))){
   ...
   # x11()
    ...
    if(k==1){ 
      ...
      par(new=TRUE)
      plot(X,M, ylim=range(c(Y,M)), axes = FALSE, xlab = "", ylab = "Robust Y")
      } 
    if(k==2){
      plot(X,Y,ylim=range(c(Y,basicMethod)))
      par(new=TRUE)
      plot(X,basicMethod, ylim=range(c(Y,basicMethod)), axes = FALSE, xlab = "", ylab = "Nadarya-Watson Y")}
